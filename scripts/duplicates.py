...
###############################################################################
# Remove duplicates from MAF files according to the following rules:
# D > W, G, X, unless higher plate number (here higher means higher number)
# https://gdac.broadinstitute.org/runs/gdc/report_2020_03_02/TCGA-SARC_Replicate_Samples.html
###############################################################################
...
maf_data['duplicate_identifier'] = (
    maf_data['Chromosome'].astype(str) + ':'
...
number_duplications = maf_data.duplicated(
    subset=['duplicate_identifier']).sum()

duplicated_variants = maf_data[
    maf_data.duplicated(subset=['duplicate_identifier'], keep = False)
    ]
...
if number_duplications > 0:
    ...
    for identifier in duplicated_variants['duplicate_identifier'].unique():
        ...
        df = duplicated_variants.loc[
            duplicated_variants['duplicate_identifier'] == identifier
            ].sort_values(
                by=['plate_number', 'analyte_code'],
